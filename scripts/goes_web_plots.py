...
def main(args):
    ...
    for region in regions.keys():
        try:
            ...
            print('Getting climatology based temp limits.')
            v0,v1 = get_climatology_limits(extent, climatology_full)
            print(f'Climatology: {v0}-{v1}')
            ...
            print(f"Updated temp range: {regions[region]['minT']}-{regions[region]['maxT']}")
            if regions[region]['maxT'] - regions[region]['minT'] < 2:
                print('Provided temperature range <2C, defaulting back to climatology.')
                regions[region]['minT'] = v0
                regions[region]['maxT'] = v1
            print(f"Final temp range: {regions[region]['minT']}-{regions[region]['maxT']}")
